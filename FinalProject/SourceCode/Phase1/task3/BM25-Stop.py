...
#DESCRIPTION: This File is used to run the BM25 Algorithm on a stopped corpus 
#             as well as query. The output is present in BM25Stop-Output.txt

#Global Constants
os.chdir("..")
os.chdir("..")
InputFolder = path = os.path.join(os.getcwd(),'corpus_stopped')
k1 = 1.2
k2 = 100
b = 0.75
...
def main():
    ...
    results = {}
    for qid in range(0, len(queries)): # BM25 scoring for each of the queries
        qarr = queries[qid].split()
        query = []
        for q in qarr:
            if q not in stopList:
                query.append(q)
        results[qid] = (run_bm25(uni_index,relevant,query, docWC, qid+1))
   
    #Obtaining the top 100 results for every query
    topresults = {}
    os.chdir("Phase1/task3")
    f = open("BM25-Stop-Output.txt", "w+")
    for r in results:
        topresults = results[r]
        rank = 1
        for i in sorted(topresults, key=topresults.get, reverse=True)[:100]:
            f.write(str(r+1) + " Q0 " + str(i)[:-4] + " " + str(rank) + " " + str(topresults[i]) + " BM25-Stop-JSY" + "\n")
            rank += 1
    f.close()
