...
#DESCRIPTION: This File is used to run the base line BM25 Algorithm and produces
#             output for the queries in BM25-Output.txt
...
def main():
    ...
    f = open("BM25-Output.txt", "w")
    for r in results:
        topresults = results[r]
        rank = 1
        for i in sorted(topresults, key=topresults.get, reverse=True)[:100]:
            f.write(str(r+1) + " Q0 " + str(i)[:-4] + " " + str(rank) + " " + str(topresults[i]) + " BM25-JSY" + "\n")
            rank += 1
    f.close()
