...
#DESCRIPTION: This File is used to run the base line TfIdf Algorithm and produces
#             output for the queries in TfIdf-Output.txt
...
def main():
    ...
    topresults={}
    os.chdir("Phase1/task1")
    f = open("TfIdf-Output.txt", "w")
    for r in results:
        topresults = results[r]
        rank = 1
        for i in sorted(topresults, key=topresults.get, reverse=True)[:100]:
            f.write(str(r+1) + " Q0 " + str(i)[:-4] + " " + str(rank) + " " + str(topresults[i]) + " TfIdf-JSY" + "\n")
            rank+=1
    f.close()
